...
- **`isSupported` is not `isActive`.** Safari has had `navigator.wakeLock` since iOS 16.4, but its native lock is flakier than Chromium's about actually holding — it can silently fail or get dropped without a catchable rejection. The composable's `startAudioFallback()` (`app/composables/useWakeLock.ts`) must gate on VueUse's `isActive` (a lock is actually held right now), not `isSupported` (the API merely exists) — gating on `isSupported` meant modern iPhones never got the fallback even when the native lock silently failed. This isn't something `@vueuse/core`'s `useWakeLock` provides — the AudioContext fallback trick is hand-rolled; VueUse only wraps the native API. Worth upstreaming to VueUse at some point.
